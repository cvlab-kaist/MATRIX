...
class MATRIXWriter(): 
    def __init__(
        ...
        sub_metric_rows=["f2t_tp", "f2t_tn", "f2t_fp", "f2t_fn"]
    ...
    def _update_f2v(self, attn_weights, layer, timestep_idx, mask_dict, query_idx=0, word="", frame_idx=None, device=0, mode="default"):
        ...
        for key, r in ranges.items():
            ...
            f2v_masked = f2v_masked.reshape(1, selected_frames, self.latent_h, self.latent_w)
            f2v_nonmasked = f2v_nonmasked.reshape(1, selected_frames, self.latent_h, self.latent_w)
