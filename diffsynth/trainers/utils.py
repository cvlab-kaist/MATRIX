...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def switch_pipe_to_training_mode(
        ...
        for p in pipe.dit.patch_embedding.parameters():
            p.requires_grad_(True)
        if "seg_head" in trainable_models.split(","):
            for p in pipe.dit.seg_head.parameters():
                p.requires_grad_(True)
...
def get_parser():
    ...
    parser.add_argument("--v2t_layers", type=str, default=None)
    parser.add_argument("--v2v_layers", type=str, default=None)
