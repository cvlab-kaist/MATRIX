...
from ..trainers.seg_head import SegHead
try:
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, is_v2v_layer=False, spa_loss=False):
        ...
        height = 30
        width = 52
        if is_v2v_layer and spa_loss:
            scale = q.shape[-1]
            print(q.shape, k.shape)
            self.f2v_weights = torch.matmul(
                q[:, :, : height * width, :],
                k.transpose(-2, -1)
            ) * (1.0 / scale ** 0.5)

            self.v2f_weights = torch.matmul(
                q,
                k[:, :, :height * width, :].transpose(-2, -1)
            ) * (1.0 / scale ** 0.5)
...
class CrossAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6, has_image_input: bool = False):
        ...
        self.norm_q = RMSNorm(dim, eps=eps)
        self.norm_k = RMSNorm(dim, eps=eps)
        self.has_image_input = False # has_image_input
        if has_image_input:
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, is_v2t_layer=False, sga_loss=False):
        ...
        x = self.attn(q, k, v)
        # print(q.shape, k.shape)
        if is_v2t_layer and sga_loss:
            scale = q.shape[-1]
            self.v2t_weights = torch.matmul(
                q,
                k.transpose(-2, -1)
            ) * (1.0 / (scale ** 0.5))
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, is_v2t_layer:bool=False, is_v2v_layer:bool=False, sga_loss:bool=False, spa_loss:bool=False):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs, is_v2v_layer=is_v2v_layer, spa_loss=spa_loss))
        x = x + self.cross_attn(self.norm3(x), context, is_v2t_layer=is_v2t_layer, sga_loss=sga_loss)
